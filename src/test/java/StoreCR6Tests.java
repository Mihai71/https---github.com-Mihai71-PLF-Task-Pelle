...
class StoreCR6Tests {
    ...
    // 2) Card: no 5 HUF rounding, CR7 sets card bonus to 0% (was 0.5%), then 0.1 HUF rounding
    @Test
    void test_cr6_card_1kg_apple_bonus_and_rounding() {
        ...
        assertEquals(roundToTenth(500.0 /* * 0.995 — CR7: bonus=0% */), price.getAmount(), 0.0001);
        assertEquals(List.of(), price.getUnusedCoupons());
    }

    // 3) Card + quantity discount (2 kg bananas → 10%); CR7: bonus=0%
    @Test
    ...
    void test_cr6_card_coupon_then_bonus() {
        ...
        assertEquals(roundToTenth(450.0 /* * 0.995 — CR7: bonus=0% */), price.getAmount(), 0.0001);
        assertEquals(List.of(), price.getUnusedCoupons());
    }

    // 7) Card + X10 (non-combinable); CR7: bonus=0%
    @Test
    void test_cr6_card_x10_noncombinable_then_bonus() {
        ...
        assertEquals(roundToTenth(1450.0 * 0.90 /* * 0.995 — CR7: bonus=0% */), price.getAmount(), 0.0001);
    ...
    void test_cr6_cash_vs_card_same_cart() {
        ...
        assertEquals(roundToTenth(450.0 /* * 0.995 — CR7: bonus=0% */), card.getAmount(), 0.0001);
    }

    // 9) Card: quantity discount (3 kg bananas → 10%); CR7: bonus=0%
    @Test
    void test_cr6_card_large_cart_quantity_discount() {
        Cart cart = new Cart(List.of(new Item(Product.BANANA, 3.0))); // 1350 → 10% = 1215
        double after = 1215.0 /* * 0.995 — CR7: bonus=0% */;
        PriceInfo price = target.getCartPrice(cart, normal, List.of(), PaymentMethod.CARD);
        assertEquals(roundToTenth(after), price.getAmount(), 0.0001);
    }
